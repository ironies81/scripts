...
function Show-Menu {
    ...
    $btn4.Add_Click({ ModifyIP; $form.Close() })
...
function Show-IpConfigAll {
    try {
        ...
        $btnOK.Add_Click({ $form.Close(); Show-Menu })
...
function Enable-DHCP {
    try {
        ...
        # Supprimer les adresses IP manuelles
        ...
        # Supprimer les routes manuelles (passerelles definies manuellement)
        ...
        # Relancer les connexions reseau
        ipconfig /release | Out-Null
        Start-Sleep -Seconds 2
        ipconfig /renew | Out-Null
        Start-Sleep -Seconds 2
...
function Launch-DNSResolverWindow {
    ...
    $btnOK.Add_Click({
        ...
        if (![string]::IsNullOrWhiteSpace($domain)) {
            # Lancer nslookup dans une fenetre PowerShell externe
